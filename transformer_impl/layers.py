...
def qkv_attention(query, key, value, mask=None, dropout=None):
    ...
    if mask is not None:
        # Fills elements of the tensor with value (-1e9) where mask==0 is true.
...
class Embeddings(nn.Module):
    ...
    def forward(self, x):
        # this embedding is multiplied by sqrt(d_model) so it has roughly the 
        # same scale as the positional encoding 
